...
def func_detect_fire_smoke():
    CLASSES_CUSTOM = [ 'fire', 'smoke']
    ...
    def get_yolo5(label):
        return torch.hub.load('ultralytics/yolov5', 'custom', path='smoke_fire_m.pt')  
    ...
    model_type = st.sidebar.selectbox('Select model type', ('Custom',), index=0)

    with st.spinner('Loading the model...'):
        cache_key = 'smoke_fire'
    ...
    classes_selector = st.sidebar.multiselect('Select classes', CLASSES, default='fire')
    ...
    if prediction_mode == 'Single image':
        ...
        agree = st.checkbox("Enable fire logging", value=False)
        if agree:
